...
class MapperModelBuilder {
  ...
  static List<MapperModel> buildCustomMapper(ValidationContext context,
    ...
    final id = mapper.type != null
        ? MapperId.fromImplementationClass(mapper.type!.name)
        : (mapper.instance != null
            ? MapperId.fromInstantiationCode(toCode(mapper.instance!))
            : MapperId.fromInstanceName(mapper.name!));

    var instanceName = mapper.name;
    var implementationClass = mapper.type?.name;
  ...
  static List<MapperModel> _buildLiteralClassMapper(
    ...
    final mappedClassModel = MappedClassModelBuilder.buildMappedClassModel(
        ...
        literalInfo.fields,
        (field) => field.isRdfValue || field.isRdfLanguageTag);

    final isMethodBased =
        fromLiteralTermMethod != null && toLiteralTermMethod != null;

    if (!isMethodBased &&
        mappedClassModel.allRdfFields
            .any((p) => !(p.isRdfLanguageTag || p.isRdfValue))) {
      context.addError(
        'LiteralMapper must only have Value or LanguagePart part parameters, but found: ${mappedClassModel.allRdfFields.where((p) => !(p.isRdfLanguageTag || p.isRdfValue)).map((p) => p.name)}',
      );
      return [];
    }
