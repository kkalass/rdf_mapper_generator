...
void main() {
  group('Resource Processor Validation Tests', () {
    ...
    setUpAll(() async {
      ...
      (libraryElement, _) = await analyzeTestFile('valid_generic_test_models.dart');
    });
    ...
    test('buildTemplateDataFromString method exists and can handle valid classes', () {
    ...
    test('buildTemplateDataFromString function exists and is properly typed', () async {
...
''';
      ...
      try {
        await buildTemplateDataFromString(emptySource);
        fail('Expected exception since no RDF annotations should result in null template data');
      } catch (e) {
        expect(e.toString(), contains('Template data was null'));
      }
    });
  });
}