...
void main() {
  group('Generic Types Integration Tests', () {
    test('build succeeds with valid generic classes', () async {
      ...
      final generatedFile = File('test/fixtures/valid_generic_test_models.rdf_mapper.g.dart');
      
      expect(generatedFile.existsSync(), isTrue, 
        reason: 'Generated mapper file should exist for valid generic classes');
      ...
      expect(content, contains('implements GlobalResourceMapper<GenericDocument<T>>'));
      expect(content, contains('class MultiGenericDocumentMapper<T, U, V>'));
      expect(content, contains('implements GlobalResourceMapper<MultiGenericDocument<T, U, V>>'));
      expect(content, contains('class GenericLocalResourceMapper<T>'));
      expect(content, contains('implements LocalResourceMapper<GenericLocalResource<T>>'));
    ...
    test('build succeeds with valid generic classes only', () async {
      ...
      expect(result.exitCode, equals(0), 
        reason: 'Build should succeed when fixture files contain only valid generic classes');
      ...
      expect(allOutput, isNot(contains('must have registerGlobally set to false')));
      expect(allOutput, isNot(contains('Generic classes cannot be registered globally')));
    });

    test('original document example still works', () async {
      // Verify that our changes didn't break the existing document example
      final generatedFile = File('test/fixtures/rdf_mapper_annotations/examples/document_example.rdf_mapper.g.dart');
      ...
      expect(content, isNot(contains('GlobalResourceMapper<Document<T>><Document<T>>')));
    });
    ...
    test('project builds cleanly without validation errors', () async {
      ...
      expect(result.exitCode, equals(0),
        reason: 'Project should build cleanly after removing invalid test classes');
    ...
    test('generated code compiles without issues', () async {
      // This test verifies that the generated code is syntactically correct
      // by running the analyzer on it
      
      final result = await Process.run(
        'dart', 
        ['analyze', 'test/fixtures/valid_generic_test_models.rdf_mapper.g.dart'],
        workingDirectory: Directory.current.path,
      );
