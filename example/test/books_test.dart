import 'package:example/books.dart';
import 'package:example/books.rdf_mapper.g.dart';
import 'package:example/init_rdf_mapper.g.dart';
    ...
    group('Book class (complete integration)', () {
      ...
      test('round-trip serialization maintains data integrity', () {
        ...
        final originalBook = Book(
          ...
          chapters: [Chapter('Chapter One', 1), Chapter('Chapter Two', 2)],
        );

        // Serialize
        final (subject, triples) = bookMapper.toRdfResource(
          originalBook,
          serContext,
        );
        ...
        // Deserialize
        final deserializedBook = bookMapper.fromRdfResource(
          subject,
          deserContext,
        );
        ...
        expect(
          deserializedBook.rating.stars,
          equals(originalBook.rating.stars),
        );
        expect(
          deserializedBook.chapters.length,
        ...
        for (int i = 0; i < originalChapters.length; i++) {
          expect(
            deserializedChapters[i].title,
            equals(originalChapters[i].title),
          );
          expect(
            deserializedChapters[i].number,
